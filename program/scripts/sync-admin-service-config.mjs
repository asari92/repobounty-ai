...
async function main() {
  ...
  if (!existingConfig) {
    await program.methods
      .initializeConfig(serviceKeypair.publicKey, serviceKeypair.publicKey)
      .accounts({
        config: configPda,
  ...
  if (existingConfig.admin.toBase58() !== adminKeypair.publicKey.toBase58()) {
    throw new Error(
      `config admin ${existingConfig.admin.toBase58()} does not match current admin wallet ${adminKeypair.publicKey.toBase58()}`
    );
  }
  ...
  if (finalizeMatches && claimMatches) {
    console.log('Config already matches admin/service wallet roles.');
    return;
  }

  await program.methods
    .updateConfig(null, serviceKeypair.publicKey, serviceKeypair.publicKey, null)
    .accounts({
      config: configPda,
      admin: adminKeypair.publicKey,
    })
  ...
  console.log('Updated config finalize_authority and claim_authority to service wallet.');
}
