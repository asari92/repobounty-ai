...
pub struct ClaimUserPaid<'info> {
    ...
    /// User who pays the transaction fee.
    #[account(mut)]
    pub user: Signer<'info>,

    /// Backend co-signer for authorization (prevents unauthorized claims).
...
pub fn handler(ctx: Context<ClaimUserPaid>) -> Result<()> {
    let clock = Clock::get()?;
